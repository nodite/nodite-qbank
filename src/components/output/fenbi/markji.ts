...
export default class Markji extends MarkjiBase {
  ...
  protected async __meta2Output(question: any, meta: any): Promise<AssetString> {
    ...
    if (meta?.materials && !lodash.isEmpty(meta.materials)) {
    ...
    if (meta?.options && !lodash.isEmpty(meta.options)) {
      textAssembler.push(
      ...
      _meta.content.text +=
        ...
        lodash
          .map(_optionAccessory.options, (option, idx) => {
            return `${String.fromCodePoint(65 + Number(idx))}. ${lodash.trim(option)}`
          })
          .join('\n')
    }
  ...
  protected async _processChoice(question: any, qbank: QBankParams): Promise<AssetString> {
    ...
    if (!lodash.isEmpty(_options?.options)) {
      _meta.options.push(
        ...lodash.map(_options?.options, (option) => ({
          assets: [] as never,
          text: lodash.trim(option),
        })),
      )
    }
    ...
    if (!lodash.isEmpty(_optionsHtml?.options)) {
      ...
      for (const _op of _optionsHtml.options) {
        ...
        const _pasedOp = lodash.trim(await fenbi.parseDoc(_op))
        _options.push(`${point}. ${_pasedOp}`)
        _meta.options.push({assets: [] as never, text: lodash.trim(point)})
      }
    ...
    if (question.correctAnswer.type === 201) {
      ...
      _meta.options = lodash.map(_meta.options, (option) => ({
        ...
        text: `${_meta.answers.includes(option) ? '*' : '-'} ${lodash.trim(option.text)}`,
      }))
    } else {
      throwError('Unsupported correct answer type.', {qbank, question})
    }
